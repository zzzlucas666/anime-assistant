...
- 事件事实来源校验、类型化生命周期、中文语义检索和长期对话摘要。
- 根据重要事件、情绪和空闲时间主动发起对话。
- 可选 Live2D 立绘、情绪参数映射和真实音频振幅嘴型。
- 可选 Mio 本地 Style-Bert-VITS2 / AivisSpeech 日语语音；中文回复在后台转成自然日语后发声。
- JSON 原子保存、备份恢复和本地日志。

语义模型会在启动后后台预热；预热完成前自动使用轻量词面检索，不阻塞聊天首字。模型就绪后，低优先级任务会分批回填缺失或版本过期的事件向量。事件提取和长期摘要在顺序后台队列中执行；摘要默认每累计 10 条溢出消息批量生成一次。

事件记忆采用 Memory Record v2。AI 提取结果必须携带可在用户原话中逐字验证的证据，才能成为 `confirmed` 记忆；无法证明来源的内容只作为短期 `candidate` 保存，不会进入对话提示、主动聊天或关系更新。记忆按 `type` 使用不同过期规则，用户明确纠正的档案会覆盖旧值，同时保留旧事实的失效历史。

普通聊天、启动问候和主动聊天共用同一个五层 Prompt 构造器。Python 先把关系数值、情绪状态和可信记忆转换成语义行为，再交给模型自然表达；原始好感/信任分数不会直接进入 Prompt。只有用户明确证据属于高可信事实，系统观察和旧迁移记录会标为中可信背景，AI 推测候选仍被完全排除。系统观察默认只可用于背景检索，不能直接改变关系数值或覆盖用户明确档案。
